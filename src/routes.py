...
def search():
    ...
    if key:
        title = key
        items = (
            Product.query.filter(Product.product_name.like("%" + key + "%"))
            .order_by(Product.product_id.desc())
            .all()
        )
...
def profile():
    ...
    user_log = User_Logs.query.filter_by(log_user_id=current_user.user_id).first()

    profile_img = url_for("static", filename="profile_pics/" + user.profile_pic)
    ...
    if form.validate_on_submit():
        ...
        if logged_user and password == logged_user.password:
            ...
            user_log = User_Logs.query.filter_by(
                log_user_id=current_user.user_id
            ).first()
...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(
            regged_user.user_id, fullname, gender, phone, address, profile_pic
        )
...
def add_item():
    ...
    if form.validate_on_submit():
        ...
        product = (
            Product.query.filter_by(product_user_id=user_id)
            .order_by(Product.product_id.desc())
            .first()
        )
...
def item(item_id):
    ...
    bets = (
        Bets.query.filter_by(bet_product_id=item_id)
...
def user_items(user_id):
    page_num = request.args.get("page", 1, int)
    items = (
        Product.query.filter_by(product_user_id=current_user.user_id)
        .join(Product_Status, Product.product_id == Product_Status.product_id)
        .order_by(Product.product_id.desc())
        .add_columns(
            Product.product_name,
            Product.product_id,
            ...
            Product.product_pic,
            Product_Status.condition,
            Product_Status.entry_date,
            Product_Status.entry_time,
            Product_Status.last_update_date,
            Product_Status.last_update_time,
        )
        .paginate(page=page_num, per_page=10)
    )
    total = items.total
    return render_template(
        "item_list.html", title="My Products", items=items, total=total
    )
...
def item_delete(item_id):
    product = Product.query.filter_by(
        product_id=item_id, product_user_id=current_user.user_id
    ).first()
    if product:
